...
export default function BrainTumorAnalysis() {
  ...
  ${hasTumor ? 'The scan indicates presence of a tumor. ' : 'The scan shows no tumor. '}Provide a detailed analysis with appropriate urgency level based on the detection result and confidence score.`
      ...
      try {
        ...
        // Force correct urgency level based on detection and confidence
        if (hasTumor || prediction.toLowerCase().includes('tumor') || prediction.toLowerCase().includes('yes')) {
          if (confidence > 80) {
            analysis.urgencyLevel = 'high';
          } else if (confidence > 60) {
            analysis.urgencyLevel = 'medium';
          } else {
            analysis.urgencyLevel = 'low'; // Only for confidence <= 60%
          }
          ...
          // For negative cases, set urgency to low unless specified otherwise
