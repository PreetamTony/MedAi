...
export default function HealthEducation() {
  ...
  const searchMedicalInfo = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('https://api.groq.com/openai/v1/chat/completions', {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: 'system',
              content: 'You are a medical education assistant. Provide clear, accurate, and structured medical information without markdown symbols like **. Use simple language and include relevant research when available.'
            },
            {
              role: 'user',
              content: query
      ...
      const formattedResponse = data.choices[0].message.content.replace(/\*\*/g, ''); // Remove markdown symbols
